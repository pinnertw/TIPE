...
%% Exemple: minimax
        ...
        \begin{center}
            \begin{tikzpicture}[level distance=1.3cm,
                ...
                \node (n){\showfullgoban \black{b2}}
                child {node (n0){\only<5-20>{\showfullgoban} \only<21->{27} \white{a2}}
                    child {node (n1){\only<6-12>{\showfullgoban} \only<13->{27}\black{a3}}
                        child {node (n7){\only<7>{\showfullgoban} \only<8->{4}}
                        \clear{a3}
                        \black{b3}}
                        child {node (n2){\only<9>{\showfullgoban} \only<10->{27}}
                        \clear{b3}
                        \black{c3}}
                        child {node (n8){\only<11>{\showfullgoban} \only<12->{13}}
                        \clear{c3}}
                        }
                    child {node (n3){\white{a3} \only<6-19>{\showfullgoban} \only<20->{29} \black{a2}}
                        child {node (n9){\only<14>{\showfullgoban} \only<15->{2}}
                        \clear{a2}
                        \black{a1}}
                        child {node (n10){\only<16>{\showfullgoban} \only<17->{23}}
                        \clear{a1}
                        \black{b1}}
                        child {node (n4){\only<18>{\showfullgoban} \only<19->{29}} \cleargoban \black{a2}}
                        }
                    }
                    child {node (n5){\uncover<5->{\showfullgoban}}
                    child {node (n6){$\vdots$}}
                };
                ...
                \only<13->{\draw[->,draw=red] (n2) -- (n1);}
                \only<20->{\draw[->,draw=red] (n4) -- (n3);}
                \only<21->{\draw[->,draw=red] (n1) -- (n0);}
                \only<7->{\draw[-] (n7) -- (n1);}
                \only<9-12>{\draw[-] (n2) -- (n1);}
                \only<11->{\draw[-] (n8) -- (n1);}
                \only<14->{\draw[-] (n9) -- (n3);}
                \only<16->{\draw[-] (n10) -- (n3);}
                \only<18-19>{\draw[-] (n4) -- (n3);}
                ;
            \end{tikzpicture}
        \end{center}
    }
%% Exemple?
    \uncover<22>{En pratique, on se donne une hauteur et on fait un parcours en profondeur.}
    \cleargoban
\end{frame}
...
\begin{itemize}
    ...
    \item<4-> k = 4\\
        $m \ge 5$, $n \ge 5$
\end{itemize}
\uncover<5->{Dans le cas où $k \ge 8$, il n'existe pas de stratégie gagnante pour le premier joueur.}
\end{frame}
