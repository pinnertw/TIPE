...
%% Exemple: note 
    \begin{center}
        ...
        \shortstack{\showgoban}
    \end{center}
    \center{La note au point A vaut 90 (81 + $3\times 3$).}
\end{frame}
...
%% Exemple?
    \uncover<5>{En pratique, on se donne une hauteur et on fait un parcours en profondeur.}
\end{frame}
...
%% m-n-8
    \includegraphics[height=5cm]{8-sol.eps}
...
\uncover<2->{Dans le cas où $k \le 5$, une telle stratégie existe lorsque}
\begin{itemize}
    \item<3-> k = 3\\
        $m \ge 3$, $n \ge 4$
    \item<4-> k = 4\\
